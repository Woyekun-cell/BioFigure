...
out_root <- if (length(args)) normalizePath(args[[1]], mustWork = FALSE) else file.path(tempdir(), "biofigure-064-080")
fig_dir <- file.path(out_root, "figures")
dat_dir <- file.path(out_root, "plot_data")
work_dir <- file.path(out_root, "work", "widgets")
...
# 083 / issue 072 — test webr runtime, then use a stable cumulative-angle fallback if needed.
...
writeLines(capture.output(sessionInfo()),file.path(out_root,"tables","sessionInfo_071_090.txt"))
