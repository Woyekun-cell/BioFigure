...
dens <- ggplot(d,aes(value,Genus,fill=after_stat(x)))+geom_density_ridges_gradient(scale=1.2,rel_min_height=.015,colour="#4A4A4A",linewidth=.25)+scale_fill_gradient2(low="#397CB5",mid="#F7F7F4",high="#75C69D",midpoint=0)+scale_x_continuous(limits=c(-.055,.055),breaks=c(-.03,0,.03))+labs(x="β1 (MAG)",y=NULL)+theme_classic(base_size=12,base_family=font)+theme(text=element_text(family=font),legend.position="none",axis.text.y=element_text(size=9.2),axis.line.y=element_blank(),axis.ticks.y=element_blank(),plot.margin=margin(5,0,5,5))
b <- d |> mutate(Direction=ifelse(value>0,"Positive","Negative")) |> count(Genus,Direction) |> mutate(n=ifelse(Direction=="Negative",-n,n))
bars <- ggplot(b,aes(n,Genus,fill=Direction))+geom_col(colour="#333333",linewidth=.25,width=.82)+geom_vline(xintercept=0,linewidth=.4)+scale_fill_manual(values=c(Negative="#397CB5",Positive="#75C69D"))+scale_x_continuous(breaks=c(-20,-10,0,10),labels=abs,expand=c(0,0))+coord_cartesian(xlim=c(-20,12),clip="off")+labs(x="Number of MAGs",y=NULL)+theme_classic(base_size=12,base_family=font)+theme(text=element_text(family=font),legend.position="none",axis.text.y=element_blank(),axis.ticks.y=element_blank(),axis.line.y=element_blank(),plot.margin=margin(5,5,5,0))
...
ragg::agg_png("results/figures/scidraw_ridge_bipolar.png",width=203,height=180,units='mm',res=300,background='white');print(p);dev.off()
