...
class CardTests(unittest.TestCase):
 ...
 def test_source_compared_methods_lock_specialized_packages_and_alignment(self):
  ...
  self.assertEqual(by_id['SCIDRAW-PCOA-MARGINAL-001']['runtime_status'],'source-compared-png')
  self.assertTrue(all(x['review_status'] in {'pass','needs-revision'} for x in doc['cases']))
  self.assertTrue(all(x['validated_pngs'] for x in doc['cases']))
  ...
  entry=(root/'references/reproduction-learning.md').read_text()
  self.assertIn('review_status=pass',entry)
  self.assertIn('reference_analysis',entry)
  self.assertIn('不得用普通极坐标色块代替分扇区多轨环形热图',entry)
  skill=(root/'SKILL.md').read_text()
  self.assertIn('references/figure-execution-gates.md',skill)
  gates=(root/'references/figure-execution-gates.md').read_text()
  self.assertIn('读图门，未通过禁止写绘图代码',gates)
